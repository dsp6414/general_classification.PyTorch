...
class CheckpointSaver(object):
    ...
    def save_checkpoint(self, model, optimizer, cfg, epoch, model_ema=None, metric=None):
        assert epoch >= 0
        ...
        if (len(self.checkpoint_files) < self.max_history
            ...
            self._save(save_path, model, optimizer, cfg, epoch, model_ema, metric)
    ...
    def _save(self, save_path, model, optimizer, cfg, epoch, model_ema=None, metric=None):
        save_state = {
            'epoch': epoch,
    ...
    def save_recovery(self, model, optimizer, cfg, epoch, model_ema=None, use_amp=False, batch_idx=0):
        assert epoch >= 0
        filename = '-'.join([self.recovery_prefix, str(epoch), str(batch_idx)]) + self.extension
        save_path = os.path.join(self.recovery_dir, filename)
        self._save(save_path, model, optimizer, cfg, epoch, model_ema)
